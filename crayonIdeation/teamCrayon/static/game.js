...
function playRockScissorsPaper(evt) {
    ...
    var chocobee = document.getElementById('point')
    ...
    switch(source.id) {   
        //evt.target.id가 즉 buttonElement의 id가 아래와 같으면
    ...
    if (player.point == computer.point) {//같으면 비기고
        ...
        resultElement.innerHTML = '나: ' + player + ' vs 컴퓨터: ' + computer + ' 결과는?  비김';

        chocobee.innerHTML =   '!현재 점수는! ' + chocobeeSum + '개 입니다 ★';

    } else {//여긴 알고리즘부분

        if ((player.point + 1) % 3 == computer.point) {

            resultElement.innerHTML = '나: ' + player + ' vs 컴퓨터: ' + computer + ' 결과는?  짐ㅠ';

            chocobee =  '!현재 점수는! ' + chocobeeSum+ '개 입니다 ★';

        } else {

            resultElement.innerHTML = '나: ' + player + ' vs 컴퓨터: ' + computer + ' 결과는?  이김!!';
